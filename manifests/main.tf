...
resource "aws_iam_role" "cloudtrail_cloudwatch_events_role" {
  ...
  path               = "/"
  assume_role_policy = data.aws_iam_policy_document.cloudwatch-log-assume-policy.json
  tags = {
    Name        = "${var.environment}-cloudtrail-cloudwatch-events-role"
    Environment = var.environment
  }
}

resource "aws_iam_role_policy" "cloudwatch-log-policy" {
  name_prefix = "cloudtrail_cloudwatch_events_policy"
  role        = aws_iam_role.cloudtrail_cloudwatch_events_role.id
  policy      = data.aws_iam_policy_document.cloudwatch-log-policy.json
}

data "aws_iam_policy_document" "cloudwatch-log-assume-policy" {
  statement {
    effect  = "Allow"
...
data "aws_iam_policy_document" "cloudwatch-log-policy" {
  statement {
    effect  = "Allow"
    actions = ["logs:CreateLogStream"]

    resources = [
      "arn:${data.aws_partition.current.partition}:logs:${var.region}:${data.aws_caller_identity.current.account_id}:log-group:${aws_cloudwatch_log_group.cloudtrail-log-group.name}:log-stream:*",
    ]
  }
...
resource "aws_cloudwatch_log_group" "cloudtrail-log-group" {
  name = "${var.environment}-cloudtrail-cloudwatch-log-group"
...
resource "aws_cloudwatch_log_stream" "cloudtrail-log-stream" {
  name           = data.aws_caller_identity.current.account_id
  log_group_name = aws_cloudwatch_log_group.cloudtrail-log-group.name
}